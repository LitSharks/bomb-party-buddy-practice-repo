...
class Game {
  ...
  static highlightSyllable(word, syl) {
    ...
    return `${pre}<b style="font-weight:900;text-transform:uppercase;font-size:1.15em;font-family:'Noto Sans Mono','Cascadia Code','Fira Code','Source Code Pro',monospace;font-variant-ligatures:none">${mid}</b>${post}`;
  }
  ...
  _generateCandidates(context, syllable, limit) {
    ...
    const specialTypesOrder = ['foul', 'pokemon', 'minerals', 'rare'];
    ...
    const candidates = Array.from(candidateMap.values()).map(info => {
      ...
      const matchedSpecials = specialTypesOrder.filter(type => specialMatches[type]);
      const activeSpecials = matchedSpecials.filter(type => (
        type === 'foul' ? foulMode
          : type === 'pokemon' ? pokemonMode
            : type === 'minerals' ? mineralsMode
              : rareMode
      ));
      const primarySpecial = activeSpecials.length
        ? activeSpecials[0]
        : (matchedSpecials.length ? matchedSpecials[0] : null);
      const hasSpecial = matchedSpecials.length > 0;
    ...
    const comparators = {
      ...
      foul: (a, b) => {
        ...
        return (Number(b.specialMatches?.foul) - Number(a.specialMatches?.foul));
      },
      pokemon: (a, b) => {
        if (!pokemonMode) return 0;
        return (Number(b.specialMatches?.pokemon) - Number(a.specialMatches?.pokemon));
      },
      minerals: (a, b) => {
        if (!mineralsMode) return 0;
        return (Number(b.specialMatches?.minerals) - Number(a.specialMatches?.minerals));
      },
      rare: (a, b) => {
        if (!rareMode) return 0;
        return (Number(b.specialMatches?.rare) - Number(a.specialMatches?.rare));
      },
      coverage: (a, b) => {
    ...
    for (const candidate of workingCandidates) {
      ...
      for (const feature of priority) {
        ...
        if (feature === 'foul' && foulMode && candidate.specialMatches?.foul) { tone = 'foul'; break; }
        if (feature === 'pokemon' && pokemonMode && candidate.specialMatches?.pokemon) { tone = 'pokemon'; break; }
        if (feature === 'minerals' && mineralsMode && candidate.specialMatches?.minerals) { tone = 'minerals'; break; }
        if (feature === 'rare' && rareMode && candidate.specialMatches?.rare) { tone = 'rare'; break; }
      ...
      if (!tone) {
        if (candidate.primarySpecial) tone = candidate.primarySpecial;
    ...
    const candidateDetails = workingCandidates.map(c => ({
      ...
      matchedSpecials: Array.isArray(c.matchedSpecials) ? c.matchedSpecials.slice() : [],
  ...
  onCorrectWord(word, myTurn = false) {
    const pending = this._pendingSubmission
      ? { word: (this._pendingSubmission.word || '').toString() }
      : null;
    this._clearPendingSubmission();
    const normalizedInfo = this._normalizeWordForLog(word);
    const normalizedForCompare = normalizedInfo?.normalized || ((word || '').toString().trim().toLowerCase());
    const pendingNormalized = pending ? pending.word.trim().toLowerCase() : '';
    const isSelf = !!myTurn || (!!normalizedForCompare && !!pendingNormalized && normalizedForCompare === pendingNormalized);
    if (normalizedInfo) {
    ...
    } else if (isSelf && pending && pendingNormalized) {
      this._rememberWordUsage(pendingNormalized, {
        displayWord: pending.word,
        fromSelf: true,
        outcome: 'correct'
      });
    }
    ...
    const letterSource = normalizedInfo?.normalized
      || ((word || '').toString().toLowerCase())
      || pendingNormalized;
    const letters = this._lettersOf(letterSource || '');
    letters.forEach((count, c) => {
      const idx = c.charCodeAt(0) - 97;
