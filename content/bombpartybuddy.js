...
const isAsyncClipboardAllowed = (() => {
  let cached;
  return () => {
    if (cached !== undefined) return cached;
    try {
      const policy = document.policy || document.featurePolicy;
      if (policy) {
        if (typeof policy.allowsFeature === "function") {
          try {
            const allowed = policy.allowsFeature("clipboard-write");
            if (allowed === false) {
              if (policy.allowsFeature.length >= 2 && window?.location?.origin) {
                if (policy.allowsFeature("clipboard-write", window.location.origin) === false) {
                  cached = false;
                  return cached;
                }
              } else {
            ...
            // fall through to allowedFeatures check
          }
        }
        if (typeof policy.allowedFeatures === "function") {
          try {
            const allowedList = policy.allowedFeatures();
            if (Array.isArray(allowedList) && allowedList.length && !allowedList.includes("clipboard-write")) {
              cached = false;
              return cached;
            }
          } catch (_) {
            // ignore and fall back to capability check
          }
        }
      }
    } catch (_) {
      // ignore and rely on capability detection
    }
    cached = !!(navigator?.clipboard?.writeText);
    return cached;
  };
})();
...
async function copyPlain(text) {
  const payload = text ?? "";
  if (isAsyncClipboardAllowed() && navigator?.clipboard?.writeText) {
    try {
